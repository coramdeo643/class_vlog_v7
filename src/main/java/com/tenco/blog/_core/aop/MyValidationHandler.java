...
/*
동작 원리
POST, PUT 요청이 Controller 메서드에 도달하기 전에 가로채기
메서드의 매개 변수 중 Errors 객체 찾기
유효성 검사 오류가 있으면 Exception400 던지기
*/
	...
	public void validationCheck(JoinPoint joinPoint) {
		...
		for (Object arg : args) {
			...
			if (arg instanceof Errors) {
				...
				if (errors.hasErrors()) {
					// errors 에 오류가 있다면(hasErrors()) 카운트(getErrorCount())
					log.warn("유효성 검사 오류 발견 - 오류 개수 : {}", errors.getErrorCount());

					FieldError firstError = errors.getFieldErrors().get(0);
					String errorMsg = firstError.getDefaultMessage() + " : " + firstError.getField();
					throw new Exception400(errorMsg);
				}
				break;
			}
			log.debug("유효성 검사 통과 완료");
		}
